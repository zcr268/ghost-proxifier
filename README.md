...
- The release `ghost.conf` defaults to `start_minimized_to_tray=on`, so the GUI build starts silently in the tray. Set it to `off` if you want the log window to open at startup.
...
- `dns_ipv6=on` enables AAAA/IPv6 DNS results. `ipv6_connect=proxy` sends IPv6 TCP connects through the upstream SOCKS5/HTTP CONNECT proxy, including IPv6-only app sockets via the local `[::1]` relay when necessary; `ipv6_connect=direct` uses the system route and logs the direct IPv6 connect; `ipv6_connect=fail` returns quickly for non-local IPv6.
